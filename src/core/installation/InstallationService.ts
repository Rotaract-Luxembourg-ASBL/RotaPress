...
export class InstallationService {
  ...
  async isComplete(): Promise<boolean> {
    const [state] = await this.db
      .select({ completedAt: installation.completedAt })
      .from(installation)
      .where(eq(installation.id, 1));
    return Boolean(state?.completedAt);
  }

  async acceptsClaim(claim: string): Promise<boolean> {
    if (!/^[A-Za-z0-9_-]{43}$/.test(claim)) return false;
    const [state] = await this.db
      .select()
      .from(installation)
      .where(eq(installation.id, 1));
  ...
  async complete(
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (
        ...
        !state.claimExpiresAt ||
        state.claimExpiresAt.getTime() <= Date.now() ||
        state.nominatedEmail.toLowerCase() !==
          actor.email.trim().toLowerCase() ||
        !matchesClaim(claim, state.claimHash)
      ) {
        throw new DomainError(
          "SETUP_CLAIM_INVALID",
          "The setup claim is unavailable, expired, or not assigned to this verified identity.",
          409,
        );
      }
      ...
      if (!created)
        throw new Error("Organization insert did not return an identifier.");
