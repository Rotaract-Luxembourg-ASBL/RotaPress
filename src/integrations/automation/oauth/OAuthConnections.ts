...
export class OAuthConnections {
  ...
  async consent(actor: TrustedActor, headers: Headers, input: unknown) {
    ...
    if (parsed.accept) this.authorization.requireRecent(actor);
    ...
    await this.audit(
      actor,
      access.organizationId,
      parsed.accept ? "automation.oauth.authorized" : "automation.oauth.denied",
      details.clientId,
    );
  ...
  async authorizedClient(
    ...
    if (
      ...
      consent.updatedAt.getTime() + 28800000 <= Date.now() ||
  ...
  private audit(
    ...
    return new AuditRepository().record(this.db, {
      organizationId,
