...
profile. Club membership and staff permissions remain separate.
...
records are retained and event restoration is not currently supported. Forms and
Calendar activities have their own archive and restoration rules.
...
underlying collection. Counts must follow the same authorization as the records.
...
filtering, editor return and phone menus.
